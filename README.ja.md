...
5. [アプリ内アップデートチェック](#アプリ内アップデートチェック)
6. [認証とアカウント取り込み](#認証とアカウント取り込み)
7. [ワークスペース](#ワークスペース)
8. [インストール](#インストール)
9. [ソースからビルド](#ソースからビルド)
10. [Release DMG パイプライン](#release-dmg-パイプライン)
11. [プロジェクト構成](#プロジェクト構成)
12. [テスト](#テスト)
13. [トラブルシューティング](#トラブルシューティング)
14. [セキュリティとプライバシー](#セキュリティとプライバシー)
...
- `Minimal` レイアウトはカード幅を自動調整し、ウィンドウ幅に応じて列数が変わります。
- `Accounts` / `Available` / `Pool Usage` は重複 ID を考慮して集計。
- メインウィンドウのアカウントカードに利用可能なリセット情報を表示します。通常モードでは各期限を一覧表示し、`Minimal` では `リセット 2回 · 7/30, 8/1` のように短く表示します。
...
- コンパクトなアカウント一覧、グループ絞り込み、プランバッジ、フォールバック警告、リセット枠ごとの期限を表示するメニューバーエクストラ。
- 状態確認用の macOS Widget。
...
- バージョン更新後の「新機能」案内は、すべての対応アプリ言語で表示されます。
...
- デスクトップ通知とイベント履歴を提供。
- API の一時的な揺れによる誤検知を抑えるため、通知は 1 日あたりの上限があります。
...
- ポップオーバーはメイン画面のアカウント並び替え設定（現在のアカウントを先頭、有料アカウントを前、API Key を末尾）を反映。
- アカウント一覧はグループ絞り込み、Plus/Pro/API Key バッジ表示に対応し、現在のアカウントにも `切り替え` ボタンを残します。
- 行には週次/5h の残量と、取得できる場合は正確なリセット時刻を表示。
- リセット枠は `N 回リセット可能` として行内に表示し、枠ごとの期限を個別に並べます。API の `expires_at` を優先し、詳細取得に失敗した場合や日付がない場合のみ「前回の成功同期 + 30 日」で推定して警告を表示します。
- 警告はコンパクトな感嘆符ポップオーバーに収納し、縦幅を節約。
- 約 15 秒ごとに更新、手動更新も可能。

## アプリ内アップデートチェック

- GitHub の最新 Release を取得し、現在バージョンと比較します。
- `v1.8.0` と `1.8.0` のような表記差を正規化して判定します。
- インストーラはアーキテクチャ（`Apple Silicon` / `Intel`）に応じて優先選択します。
- 更新ダイアログから `Install now`、`Manual download`、`Skip this version` を実行できます。
- version/build の変更後にローカライズされた「新機能」案内を表示し、Settings から最新機能ノートを再表示できます。
