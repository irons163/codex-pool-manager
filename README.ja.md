...
- メインウィンドウのアカウントカードに利用可能なリセット情報を表示します。通常モードでは各推定期限を一覧表示し、`Minimal` では `リセット 2回 · 7/30, 8/1` のように短く表示します。
...
- コンパクトなアカウント一覧、グループ絞り込み、プランバッジ、警告ポップオーバー、リセット枠ごとの推定期限を表示するメニューバーエクストラ。
- 状態確認用の macOS Widget。
...
- リセット枠は `N 回リセット可能` として行内に表示し、枠ごとの推定期限を個別に並べます。推定は「前回の成功同期 + 30 日」なので、警告ポップオーバーで実際の期限とずれる可能性を示します。
