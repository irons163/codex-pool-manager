...
3. [主要功能](#主要功能)
4. [智慧切換運作方式](#智慧切換運作方式)
5. [Widget + 選單列](#widget--選單列)
6. [App 內更新檢查](#app-內更新檢查)
7. [驗證與帳號匯入](#驗證與帳號匯入)
8. [工作區](#工作區)
9. [安裝](#安裝)
10. [從原始碼建置](#從原始碼建置)
11. [Release DMG 流程](#release-dmg-流程)
12. [專案結構](#專案結構)
13. [測試](#測試)
14. [疑難排解](#疑難排解)
15. [安全與隱私說明](#安全與隱私說明)
...
1. **安裝**：從 [Releases](https://github.com/irons163/codex-pool-manager/releases) 下載符合 Mac 架構的 `CodexPoolManager-<version>-apple-silicon.dmg` 或 `CodexPoolManager-<version>-intel.dmg`。開啟 DMG 後，將 `CodexPoolManager` 拖到 `Applications`。
2. **開啟 App**：從 `Applications` 開啟 `CodexPoolManager`。主視窗標題是 `Dashboard`，預設會進入 `驗證` 工作區。
3. **加入第一個帳號**：在 `驗證` 工作區選擇 `OAuth / 訂閱`，按 `登入並匯入`，再依瀏覽器提示完成登入。若 Codex 已在本機登入，也可按 `掃描本機帳號`，再對找到的帳號按 `匯入`；掃描不到時按 `選擇 auth.json`。
4. **同步用量**：在主畫面上方按 `同步 Codex 用量`，等待同步完成；成功時會顯示 `上次成功同步`，失敗時請查看 `同步錯誤`。
5. **查看結果**：在 `帳號用量` 的帳號卡查看 `週用量`、`5 小時用量` 與重置時間；頂部的 `帳號`、`可用`、`池用量`、`模式` 會顯示整體狀態。需要歷史趨勢時，開啟 `用量分析`（至少成功同步一次後才會有紀錄）。
...
![主儀表板（深色，模擬資料）](docs/images/app-screenshot.png)

### 頂部總覽（淺色，模擬資料）

![頂部總覽（淺色，模擬資料）](docs/images/dashboard-light.png)
...
![Widget 空狀態範例](docs/images/widget-empty-state.png)

### OpenAI Reset Alert（模擬資料）

![OpenAI Reset Alert（模擬資料）](docs/images/openai-reset-alert.png)
...
- 主視窗帳號卡會顯示可重置額度：一般模式列出每一次期限，`極簡` 模式則以 `可重置 2 次 · 7/30, 8/1` 這類精簡日期顯示。
...
- 提供選單列工具：精簡帳號列表、群組篩選、方案徽章、警示彈出提示，以及可重置次數/逐次期限。
...
- 展開內容會沿用主視窗的帳號排序設定，包括目前帳號置頂、付費帳號優先、API Key 帳號最後。
- 帳號列表支援群組篩選，顯示 Plus/Pro/API Key 徽章，且目前帳號仍保留 `切換` 按鈕。
- 帳號列會顯示週用量與 5 小時用量的剩餘比例，並在可取得時顯示精確重置時間。
- 可重置額度會直接展開成 `可重置 N 次` 與每一次期限。程式會優先使用 API 回傳的 `expires_at`；明細請求失敗或個別日期缺失時，才以「前一次成功同步時間 + 30 天」推估並保留驚嘆號提示。
- 帳號警示收納在精簡驚嘆號彈出提示中，減少垂直空間占用。
...
- 版本或 build 變更後會顯示本地化「新功能」提示，也可從設定重新查看最新功能說明。
...
Codex Pool Manager 可以新增 API Key / 中轉帳號，供 Codex CLI custom provider 使用。中轉帳號會寫入 `~/.codex/config.toml` 的 provider 區塊，並直接更新 Codex 的 API-key `auth.json`。
