...
- 메인 창 계정 카드에 사용 가능한 초기화 정보를 표시합니다. 일반 모드는 각 예상 만료 시각을 나열하고, `Minimal` 모드는 `초기화 2회 · 7/30, 8/1`처럼 간단히 표시합니다.
...
- compact 계정 목록, 그룹 필터, 플랜 배지, 경고 popover, reset credit별 예상 만료 시간을 보여주는 메뉴바 항목
- 빠른 확인용 macOS Widget
...
- Reset credit은 `N회 리셋 가능`으로 행 안에 표시하고 credit별 예상 만료 시간을 각각 나열합니다. 예상값은 “이전 성공 동기화 시각 + 30일”로 계산하므로, 실제 만료일과 다를 수 있음을 경고 popover로 표시합니다.
