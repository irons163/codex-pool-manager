...
5. [앱 내 업데이트 확인](#앱-내-업데이트-확인)
6. [인증 및 계정 가져오기](#인증-및-계정-가져오기)
7. [워크스페이스](#워크스페이스)
8. [설치](#설치)
9. [소스에서 빌드](#소스에서-빌드)
10. [Release DMG 파이프라인](#release-dmg-파이프라인)
11. [프로젝트 구조](#프로젝트-구조)
12. [테스트](#테스트)
13. [문제 해결](#문제-해결)
14. [보안 및 개인정보](#보안-및-개인정보)
15. [기여](#기여)

## 스크린샷

아래 이미지는 모의 데이터 또는 비민감 테스트 데이터입니다.

### 메인 대시보드 (Dark, 모의 데이터)

![메인 대시보드 (Dark, 모의 데이터)](docs/images/app-screenshot.png)

### 상단 요약 (Light, 모의 데이터)

![헤더 개요 (Light, 모의 데이터)](docs/images/dashboard-light.png)

### 메뉴바 상태 (모의 데이터)

![메뉴바 상태 (모의 데이터)](docs/images/menu-bar.ko.png)

### Widget (빈 상태 예시, 모의 데이터)

![Widget 빈 상태 예시](docs/images/widget-empty-state.png)

### OpenAI Reset Alert (모의 데이터)

![OpenAI Reset Alert (모의 데이터)](docs/images/openai-reset-alert.png)
...
- `Minimal` 레이아웃은 카드 너비를 자동 조정하여 창 너비에 따라 열 개수를 바꿉니다.
- `Accounts`/`Available`/`Pool Usage` 통계는 중복 ID를 제거해 계산
- 메인 창 계정 카드에 사용 가능한 초기화 정보를 표시합니다. 일반 모드는 각 만료 시각을 나열하고, `Minimal` 모드는 `초기화 2회 · 7/30, 8/1`처럼 간단히 표시합니다.
...
- compact 계정 목록, 그룹 필터, 플랜 배지, fallback 경고, reset credit별 만료 시간을 보여주는 메뉴바 항목
- 빠른 확인용 macOS Widget
...
- 버전 업데이트 후 표시되는 “새로운 기능” 안내는 모든 지원 앱 언어로 제공됩니다.
...
- API 일시 불안정으로 인한 오탐 노이즈를 줄이기 위해 일일 알림 상한을 둡니다.
...
- popover는 현재 계정 우선, 유료 계정 우선, API Key 계정 마지막 같은 메인 창의 계정 정렬 설정을 반영합니다.
- 계정 목록은 그룹 필터, Plus/Pro/API Key 배지 표시를 지원하며 현재 계정에도 `전환` 버튼을 유지합니다.
- 행에는 주간/5시간 잔여 비율과, 확인 가능한 경우 정확한 리셋 시각을 표시합니다.
- Reset credit은 `N회 리셋 가능`으로 행 안에 표시하고 credit별 만료 시간을 각각 나열합니다. API의 `expires_at`을 우선 사용하며, 상세 요청 실패나 날짜 누락 시에만 “이전 성공 동기화 시각 + 30일”로 추정하고 경고를 표시합니다.
- 계정 경고는 compact 느낌표 popover 안에 접어 세로 공간을 줄입니다.
- 약 15초 주기 자동 갱신 + 수동 갱신 지원

## 앱 내 업데이트 확인

- GitHub 최신 Release를 조회해 현재 앱 버전과 비교합니다.
- `v1.8.0`과 `1.8.0` 같은 버전 표기 차이를 정규화해 판정합니다.
- 설치 파일은 아키텍처(`Apple Silicon` / `Intel`)에 맞춰 우선 선택합니다.
- 업데이트 대화상자에서 `Install now`, `Manual download`, `Skip this version`를 선택할 수 있습니다.
- version/build 변경 후 현지화된 “새로운 기능” 안내를 표시하며, Settings에서 최신 기능 노트를 다시 열 수 있습니다.
