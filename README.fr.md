...
5. [Vérification de mise à jour intégrée](#vérification-de-mise-à-jour-intégrée)
6. [Authentification et import de comptes](#authentification-et-import-de-comptes)
7. [Espaces de travail](#espaces-de-travail)
8. [Installation](#installation)
9. [Build depuis les sources](#build-depuis-les-sources)
10. [Pipeline Release DMG](#pipeline-release-dmg)
...
Toutes les captures ci-dessous utilisent des données de test ou non sensibles.

### Tableau principal (Dark, données de test)

![Tableau de bord principal (Dark, données de test)](docs/images/app-screenshot.png)

### Vue d'ensemble (Light, données de test)

![Vue d'ensemble de l'en-tête (Light, données de test)](docs/images/dashboard-light.png)
...
![État vide du Widget](docs/images/widget-empty-state.png)

### OpenAI Reset Alert (données de test)

![OpenAI Reset Alert (données de test)](docs/images/openai-reset-alert.png)
...
- Les prompts `Nouveautés` après mise à jour sont localisés dans toutes les langues supportées par l'app.
...
- Limite de notifications quotidienne pour réduire le bruit en cas d'instabilité API.
...
- Interroge la dernière Release GitHub et compare avec la version actuelle de l'app.
- Gère la normalisation de version (par exemple `v1.8.0` et `1.8.0`).
- Sélectionne l'installeur préféré selon l'architecture (`Apple Silicon` / `Intel`) quand disponible.
- La fenêtre de mise à jour propose `Install now`, `Manual download` et `Skip this version`.
- Affiche un prompt `Nouveautés` localisé après un changement de version/build, avec possibilité de rouvrir les dernières notes depuis Settings.
...
- sélection de l'app cible pour `Switch and launch`
...
Vérification de base des fichiers de localisation :
